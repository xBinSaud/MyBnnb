...
interface AnalyticsData {
  ...
  averageBookingAmount: number; // Add this line
...
export default function Analytics() {
  ...
  const handleYearChange = (event: SelectChangeEvent<number> | any) => {
    setYear(event.target.value as number);
  };

  const handleMonthChange = (event: SelectChangeEvent<string> | any) => {
      ...
      try {
        ...
        const daysInPeriod =
          selectedMonth === "كل الأشهر"
            ? 365
            : new Date(year, parseInt(selectedMonth), 0).getDate();
        const occupancyRate = Math.min(
          (totalDuration / daysInPeriod) * 100,
          100
        ); // Cap at 100%
        ...
        bookings.forEach((booking: any) => {
          ...
          const currentStats = bookingTypeMap.get(bookingType) || {
            count: 0,
            revenue: 0,
          };
          const amount = parseFloat(booking.amount) || 0;
        ...
        const bookingTypes = Array.from(bookingTypeMap.entries()).map(
          ([type, stats]) => ({
            type,
            count: stats.count,
        ...
        const averageExpensePerDay =
          expenses.length > 0 ? totalExpenses / daysInPeriod : 0;
    ...
    <Box sx={{ p: 3, backgroundColor: "#141b2d", minHeight: "100vh" }}>
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 4 }}>
        <FormControl size="small" sx={{ minWidth: 120 }}>
          <StyledSelect value={year} onChange={handleYearChange} displayEmpty>
            {years.map((y) => (
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={3}>
          <StyledPaper>
            ...
            <Typography variant="h4">
              {formatNumber(data.averageBookingDuration)} يوم
            </Typography>
          </StyledPaper>
        </Grid>
        <Grid item xs={12} md={3}>
          <StyledPaper>
            ...
            <Typography variant="h4">
              {formatCurrency(data.averageBookingAmount)}
            </Typography>
          </StyledPaper>
        </Grid>
        <Grid item xs={12} md={3}>
              ...
              <PieChart>
                ...
                <Tooltip
                  ...
                  formatter={(value: any, name: string, props: any) => {
                    if (!props || !data.bookingTypes[props.payload.index])
                      return [];
                ...
                {data.bookingTypes.map((type, index) => (
                  ...
                  >
                    ...
                    <Typography variant="body2">
                      {type.type} (
                      {formatPercentage(
                        (type.count / (data.totalBookings || 1)) * 100
                      )}
                      )
                    </Typography>
                  </Box>
